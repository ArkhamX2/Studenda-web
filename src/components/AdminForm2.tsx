...
const AdminForm2: FC = () => {
    ...
    const onSaveClick = async () => {
        toggle()
        setSelectedArray(await request(selectedButton,"post", selectedObject))
        setSelectedArray(await request(selectedButton,"get"))
    }

    const onDeleteClick = async () => {
        toggle()
        setSelectedArray(await request(selectedButton,"delete", selectedObject))
        setSelectedArray(await request(selectedButton,"get"))
    }
        ...
        <>
            ...
            <main style={{display:'flex', backgroundColor:'white', maxHeight: '90svh', color:'#1B0E17', boxSizing:'border-box'}}>
                <div style={{display:'flex', flexDirection:'column', border:'2px solid #490514', margin:'5px', padding:'10px', 
                    ...
                    {[...Array(RequestValue.value.length-2)].map((x, i) => {const selectedButtonId=i+1; return(
